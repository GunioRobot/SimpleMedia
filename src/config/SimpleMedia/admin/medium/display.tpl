...
<div class="z-admin-content-pagetitle">
    ...
    <h3>{$templateTitle|notifyfilters:'simplemedia.filter_hooks.media.filter'}</h3>
</div>
...
<p>
    {checkpermissionblock component='SimpleMedia::' instance='.*' level='ACCESS_EDIT'}
        ...
        <a href="{modurl modname='SimpleMedia' type='admin' func='edit' ot='medium' id=$medium.id}" title="{gt text='Edit'}" class="z-icon-es-edit">
            {gt text='Edit'}
        </a>
{*        <a href="{modurl modname='SimpleMedia' type='admin' func='edit' ot='medium' astemplate=$medium.id}" title="{gt text='Reuse for new item'}" class="z-icon-es-saveas">
    ...
    {if $medium.theFileMeta.isImage && $modvars.SimpleMedia.useThumbCropper eq true}
        ...
        {/if}
        <a href="{modurl modname='SimpleMedia' type='admin' func='editthumb' id=$medium.id thumbnr=$thumbnr returnTo='adminDisplay'}" title="{gt text='Crop thumbnail image'}">
            {img src='cropthumb.png' modname='SimpleMedia' __alt='Crop thumbnail image'} {gt text='Crop thumbnail image'}
        </a>
    {/if}
    {/checkpermissionblock}
    <a href="{modurl modname='SimpleMedia' type='admin' func='view' ot='medium'}" title="{gt text='Back to overview'}" class="z-icon-es-back">
